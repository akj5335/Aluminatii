...
export const createEvent = async (req, res) => {
  try {
    const { title, description, date, location, videoLink, virtualLink, capacity } = req.body;
    if (!title || !date) return res.status(400).json({ message: 'Title & date required' });
    const event = await Event.create({
      title, description, date, location, videoLink, virtualLink, capacity,
